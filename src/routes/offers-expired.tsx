...
export const Route = createFileRoute("/offers-expired")({ component: Page });

function Page() {
  const items = myOffers.filter((o) => o.status === "expired");
  return (
    <div>
      <PageHeader title="Offers Expired" subtitle="Expired offers" />
      {items.length === 0 ? (
        <EmptyState icon={Tag} title="No offers yet" description="Your offers expired will show up here." />
      ) : (
