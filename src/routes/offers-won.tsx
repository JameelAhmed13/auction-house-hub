...
export const Route = createFileRoute("/offers-won")({ component: Page });

function Page() {
  const items = myOffers.filter((o) => o.status === "accepted");
  return (
    <div>
      <PageHeader title="Offers Won" subtitle="Accepted offers" />
      {items.length === 0 ? (
        <EmptyState icon={Tag} title="No offers yet" description="Your offers won will show up here." />
      ) : (
