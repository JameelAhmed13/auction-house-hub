...
export const Route = createFileRoute("/current-offers")({ component: Page });

function Page() {
  const items = myOffers.filter((o) => o.status === "pending");
  return (
    <div>
      <PageHeader title="Current Offers" subtitle="Pending negotiations" />
      {items.length === 0 ? (
        <EmptyState icon={Tag} title="No offers yet" description="Your current offers will show up here." />
      ) : (
