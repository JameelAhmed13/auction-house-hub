...
export const Route = createFileRoute("/bids-lost")({ component: Page });

function Page() {
  const items = myBids.filter((b) => b.status === "lost");
  return (
    <div>
      <PageHeader title="Bids Lost" subtitle="Auctions you didn't win" />
      {items.length === 0 ? (
        <EmptyState icon={Tag} title="Nothing here yet" description="Your bids lost will appear here." />
      ) : (
