...
export const Route = createFileRoute("/current-bids")({ component: Page });

function Page() {
  const items = myBids.filter((b) => b.status === "winning" || b.status === "outbid");
  return (
    <div>
      <PageHeader title="Current Bids" subtitle="Your active bidding action" />
      {items.length === 0 ? (
        <EmptyState icon={Tag} title="Nothing here yet" description="Your current bids will appear here." />
      ) : (
